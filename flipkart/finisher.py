...
def extract_subtype_from_filename(filename):
    """Extract subtype from filename (e.g., 'coffee_table' from 'flipkart_coffee_table.json')"""
    name = filename.replace('flipkart_', '').replace('.json', '')
    return name
...
def process_output_files():
    ...
    json_files = sorted([f for f in output_dir.glob('flipkart_*.json') if 'combined' not in f.name])
    ...
    for json_file in json_files:
        ...
        try:
            ...
            for product in products:
                ...
                processed_product = {
                    'ID': f'F-{product_id_counter:04d}',
                    'Name': product_name,
                    'Dimension': clean_text(str(product.get('Dimensions', ''))),
                    'Price': product.get('Price', ''),
                    'Rating': product.get('Rating', 'NA'),
                    'ImageURL': clean_text(product.get('Image URL', '')),
...
def save_combined_output(products):
    ...
    output_file = Path(__file__).parent / 'output' / 'flipkart_combined.json'
    
    with open(output_file, 'w', encoding='utf-8') as f:
        json.dump(products, f, indent=2, ensure_ascii=False)
    
    print(f"\nCombined {len(products)} products")
    print(f"Product IDs: F-0001 to F-{len(products):04d}")
    print(f"Saved to: {output_file}")

def save_csv_output(products):
    """Save combined products to CSV for easier viewing"""
    import csv
    output_file = Path(__file__).parent / 'output' / 'flipkart_combined.csv'
    ...
    with open(output_file, 'w', newline='', encoding='utf-8') as f:
        fieldnames = ['ID', 'Name', 'Dimension', 'Price', 'Rating', 'ImageURL', 'Type', 'SubType', 'Brand', 'ProductURL']
...
if __name__ == '__main__':
    print("Starting Flipkart product finisher...\n")
