    ...
    for query in queries:
        ...
        for idx, link in enumerate(links, start=1):
            ...
            try:
                ...
                print(f"⚠️ AMAZON | Error scraping {link}: {e}")
                continue
        ...
        if not final:
            print(f"⚠️ AMAZON | No products collected for '{query}' (skipping save).")
            continue
        save_json(final, "amazon", query)
        print(f"✅ Amazon {query}: {len(final)} products")
